...
const addTeamsToPoolsSheet = (poolsSheet, teams) => {
  for (let i = 0; i < NUMBER_FIGHTS_POOLS; i++) {
    ...
    const addedRow = poolsSheet.insertRow(rowIndex, {
      ...
      K: { formula: `SUM(D${myIndex}:H${myIndex})` },
    ...
    addedRow.alignment = { vertical: 'middle', horizontal: 'center', wrapText: false }
    addedRow.getCell('A').font = getFont(11, false)
...
const populatePoolsBracketsSheet = (sheet, { teams }) => {
  ...
  sheet.columns = POOLS_BRACKETS_COLS.map(col => {
    col.hidden = ['N', 'O', 'P', 'Q', 'L'].includes(col.key); // Hide specified columns
    return col;
  });
