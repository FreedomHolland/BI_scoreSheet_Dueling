import { ARGB_LIGHT_GRAY, ARGB_WHITE, RESULTS_COLS, RESULTS_TEAM_ROW_START, STANDARD_CELL_OUTLINE, TEAM_ROW_HIGHLIGHT_PROPS, TITLE_ROW_PROPS, TOURNAMENT_TYPE_TEAMS, getFont, getFontExtended, FillRED, FillYELLOW, version_document } from './sheetConstants.js'
// sheet 1 Results
...
// ROW 1
  ...
  // ROW 2
  worksheet.addRow({ A: 'Scoring Sheet Longsword, Sword & Shield, Pole-arm' })
...
const generateHeaderRows = (worksheet, eventName, date, location) => {
  ...
  worksheet.getCell('W4').font = { color: { argb: 'FFFFFF' } };
...
const generateTeamHeaders = (worksheet, tournamentType) => {
  ...
  const row6 = worksheet.addRow({
    ...
    P: 'Received',
    Q: 'Per fight',
    R: 'H-Diff', // active versus ground difference
    S: 'Ratio', // active ground ratio read "kill/death Ratio"
    // penaties
    T: 'Y',
    U: 'R',
    V: 'Total'
  ...
  worksheet.getCell('T6').fill = FillYELLOW.fill
  worksheet.getCell('U6').fill = FillRED.fill
...
const generateTeamDataRows = (worksheet, teams) => {
  teams.forEach((team, index) => {
    ...
    const addedRow = worksheet.addRow({
      ...
      X: { formula: `IFERROR(COUNTIFS($W$7:$W$99,">"&W${rowIndex})+COUNTIFS($W$7:$W$99,W${rowIndex},$D$7:$D$99,">"&D${rowIndex})+COUNTIFS($W$7:$W$99,W${rowIndex},$D$7:$D$99,D${rowIndex},$M$7:$M$99,">"&M${rowIndex})+COUNTIFS($W$7:$W$99,W${rowIndex},$D$7:$D$99,D${rowIndex},$M$7:$M$99,M${rowIndex},$S$7:$S$99,">"&S${rowIndex})+COUNTIFS($W$7:$W$99,W${rowIndex},$D$7:$D$99,D${rowIndex},$M$7:$M$99,M${rowIndex},$S$7:$S$99,S${rowIndex},$V$7:$V$99,"<"&V${rowIndex}),"NA")+1` },
      ...
      Z: { formula: `IF(X${rowIndex}=3,W${rowIndex}+2,IF(X${rowIndex}=2,W${rowIndex}+4,IF(X${rowIndex}=1,W${rowIndex}+6,W${rowIndex}))) * $Z$4` },
